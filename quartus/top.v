...
module top(
    ...
    // maintain the reset signal
    reset_toggle u2 (OSC_50, ~KEY[0], ~i2c_video_done, reset_edge, LED_GREEN[8]);
    ...
    // switch
    in_port #(.WIDTH(18)) u44 (32'h80000000, clock, 1'b1, clock, address_out,
            memory_drive, bus, SW[17:0]);
    ...
    in_port u58 (32'h80000005, clock, 1'b1, clock, address_out, memory_drive,
            bus, timer_out);
    ...
    lcd u59 (clock_1_6m, clock_valid, reset_1_6m, LCD_ON, LCD_BLON,
         ...
         lcd_y, lcd_ascii);
    turn_port u60 (32'h80000010, clock, clock_1_6m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, lcd_command,
	    lcd_response);
    out_port #(.WIDTH(4)) u61 (32'h80000011, clock, clock_1_6m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, lcd_x[3:0]);
    out_port #(.WIDTH(1)) u62 (32'h80000012, clock, clock_1_6m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, lcd_y);
    out_port #(.WIDTH(8)) u63 (32'h80000013, clock, clock_1_6m, clock_valid, reset,
    ...
    turn_port u65 (32'h80000020, clock, clock_25m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, ps2_command,
	    ps2_response);
    in_port #(.WIDTH(1)) u66 (32'h80000021, clock_25m, ps2_response, clock,
            address_out, memory_drive, bus, ps2_pressed);
    in_port #(.WIDTH(8)) u67 (32'h80000022, clock_25m, ps2_response, clock,
	    address_out, memory_drive, bus, ps2_ascii);

    // sdram controller
    sdram u68 (OSC_50, clock_195k, clock_valid, reset_50m, sdram_command,
           ...
           DRAM_DQM);
    turn_port u69 (32'h80000030, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, sdram_command,
	    sdram_response);
    out_port #(.WIDTH(1)) u70 (32'h80000031, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, sdram_write);
    out_port #(.WIDTH(25)) u71 (32'h80000032, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, sdram_address);
    out_port u72 (32'h80000033, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, sdram_data_write);
    in_port u73 (32'h80000034, OSC_50, sdram_response, clock, address_out,
	    memory_drive, bus, sdram_data_read);

    // I2C configuration
    i2c_config u74 (OSC_50, clock_8_1k, clock_valid, reset_50m, I2C_SDAT,
                I2C_SCLK, i2c_audio_done, i2c_video_done);

    // speaker controller
    speaker u75 (clock_25m, clock_8_1k, clock_valid, reset_25m, i2c_audio_done,
             speaker_command, speaker_response, speaker_sample, AUD_XCK, AUD_BCLK,
             AUD_DACLRCK, AUD_DACDAT);
    turn_port u76 (32'h80000040, clock, clock_25m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, speaker_command,
	    speaker_response);
    out_port u77 (32'h80000041, clock, clock_25m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, speaker_sample);

    // microphone controller
    microphone u78 (clock_25m, clock_8_1k, clock_valid, reset_25m,
                i2c_audio_done, microphone_command, microphone_response,
                microphone_sample, AUD_BCLK, AUD_ADCLRCK, AUD_ADCDAT);
    turn_port u79 (32'h80000050, clock, clock_25m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, microphone_command,
	    microphone_response);
    in_port u80 (32'h80000051, clock_25m, microphone_response, clock, address_out,
            memory_drive, bus, microphone_sample);

    // VGA controller
    vga u81 (clock_100m, clock_valid, reset_100m,
    ...
    turn_port u82 (32'h80000060, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, vga_command, vga_response);
    out_port #(.WIDTH(1)) u83 (32'h80000061, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, vga_write);
    out_port #(.WIDTH(10)) u84 (32'h80000062, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, vga_x1);
    out_port #(.WIDTH(10)) u85 (32'h80000063, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, vga_y1);
    out_port #(.WIDTH(10)) u86 (32'h80000064, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, vga_x2);
    out_port #(.WIDTH(10)) u87 (32'h80000065, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, vga_y2);
    out_port #(.WIDTH(24)) u88 (32'h80000066, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, vga_color_write);
    in_port #(.WIDTH(24)) u89 (32'h80000067, OSC_50, vga_response, clock,
            address_out, memory_drive, bus,
	    {vga_color_read[23:19], 3'b0, vga_color_read[15:11],
	    3'b0, vga_color_read[7:3], 3'b0});

    // USB controller (used for both mouse and touchscreen)
    usb u90 (OSC_50, clock, clock_valid, reset_50m, OTG_ADDR, OTG_DATA, OTG_CS_N,
    ...
    // mouse controller
    turn_port u91 (32'h80000070, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, mouse_command,
	    mouse_response);
    in_port u92 (32'h80000071, OSC_50, mouse_response, clock, address_out,
            memory_drive, bus, mouse_deltax);
    in_port u93 (32'h80000072, OSC_50, mouse_response, clock, address_out,
            memory_drive, bus, mouse_deltay);
    in_port #(.WIDTH(1)) u94 (32'h80000073, OSC_50, mouse_response, clock,
            address_out, memory_drive, bus, mouse_button1);
    in_port #(.WIDTH(1)) u95 (32'h80000074, OSC_50, mouse_response, clock,
            address_out, memory_drive, bus, mouse_button2);
    in_port #(.WIDTH(1)) u96 (32'h80000075, OSC_50, mouse_response, clock,
            address_out, memory_drive, bus, mouse_button3);

    // touchscreen controller
    turn_port u97 (32'h800000e0, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, touch_command,
	    touch_response);
    in_port #(.WIDTH(10)) u98 (32'h800000e1, OSC_50, touch_response, clock,
            address_out, memory_drive, bus, touch_x);
    in_port #(.WIDTH(9)) u99 (32'h800000e2, OSC_50, touch_response, clock,
            address_out, memory_drive, bus, touch_y);
    in_port #(.WIDTH(1)) u100 (32'h800000e3, OSC_50, touch_response, clock,
            address_out, memory_drive, bus, touch_pressed);

    // SD controller
    sd u101 (OSC_50, clock_valid, reset_50m, SD_CMD, SD_DAT[3], SD_DAT[0],
	     SD_CLK, sd_command, sd_response, sd_write, sd_address,
	     sd_data_write, sd_data_read);
    turn_port u102 (32'h80000080, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, sd_command, sd_response);
    out_port #(.WIDTH(1)) u103 (32'h80000081, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, sd_write);
    out_port #(.WIDTH(30)) u104 (32'h80000082, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, sd_address);
    out_port u105 (32'h80000083, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, sd_data_write);
    in_port u106 (32'h80000084, OSC_50, sd_response, clock, address_out,
            memory_drive, bus, sd_data_read);

    // serial port receive controller
    serial_receive u107 (OSC_50, clock_valid, reset_50m, UART_RXD,
                    serial_receive_command, serial_receive_response,
                    serial_receive_data);
    turn_port u108 (32'h80000090, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, serial_receive_command,
	    serial_receive_response);
    in_port #(.WIDTH(8)) u109 (32'h80000091, OSC_50, serial_receive_response,
            clock, address_out, memory_drive, bus, serial_receive_data);

    // serial port send controller
    serial_send u110 (OSC_50, clock_valid, reset_50m, UART_TXD,
                 serial_send_command, serial_send_response, serial_send_data);
    turn_port u111 (32'h800000a0, clock, OSC_50, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, serial_send_command,
	    serial_send_response);
    out_port #(.WIDTH(8)) u112 (32'h800000a1, clock, OSC_50, clock_valid, reset,
    ...
    // on the VGA), and assign TD_RESET_N = 1'b1 (otherwise i2c_config
    // won't be able to initialize the video).
    camera u113 (clock_100m, clock_valid, reset_100m, i2c_video_done,
    ...
    turn_port u114 (32'h800000b0, clock, clock_100m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, camera_command,
	    camera_response);
    out_port #(.WIDTH(10)) u115 (32'h800000b1, clock, clock_100m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, camera_x);
    out_port #(.WIDTH(10)) u116 (32'h800000b2, clock, clock_100m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, camera_y);
    out_port #(.WIDTH(2)) u117 (32'h800000b3, clock, clock_100m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, camera_scale);
    out_port #(.WIDTH(1)) u118 (32'h800000b4, clock, clock_100m, clock_valid, reset,
            address_out, memory_drive, memory_write, bus, camera_mirror);

//    // fft controller (disabled by default; uncomment if needed)
//    fft u119 (OSC_50, clock_valid, reset_50m, fft_send_command, fft_send_response,
...
//    turn_port u120 (32'h800000c0, clock, OSC_50, clock_valid, reset,
//            address_out, memory_drive, memory_write, bus, fft_send_command,
//	    fft_send_response);
//    out_port #(.WIDTH(16)) u121 (32'h800000c1, clock, OSC_50, clock_valid, reset,
//            address_out, memory_drive, memory_write, bus, fft_send_data_real);
//    out_port #(.WIDTH(16)) u122 (32'h800000c2, clock, OSC_50, clock_valid, reset,
//            address_out, memory_drive, memory_write, bus, fft_send_data_imag);
//    out_port #(.WIDTH(1)) u123 (32'h800000c3, clock, OSC_50, clock_valid, reset,
//            address_out, memory_drive, memory_write, bus, fft_send_inverse);
//    out_port #(.WIDTH(1)) u124 (32'h800000c4, clock, OSC_50, clock_valid, reset,
//            address_out, memory_drive, memory_write, bus, fft_send_end);
//
//    turn_port u125 (32'h800000d0, clock, OSC_50, clock_valid, reset,
//            address_out, memory_drive, memory_write, bus, fft_receive_command,
//	    fft_receive_response);
//    in_port u126 (32'h800000d1, OSC_50, fft_receive_response, clock, address_out,
//            memory_drive, bus,
//            {
...
//	    }
//	);
//    in_port u127 (32'h800000d2, OSC_50, fft_receive_response, clock, address_out,
//            memory_drive, bus,
//            {
//	    // sign extend the result to 32 bits
...
//	);
//
//    // PWM controller (untested since switching to turn protocol)
//    pwm u128 (clock_100m, clock_valid, reset_100m, pwm_command, pwm_response,
//              pwm_period, pwm_high, GPIO[2]);
//    turn_port u129 (32'h800000f0, clock, clock_100m, clock_valid,
//            reset, address_out, memory_drive, memory_write, bus, pwm_command,
//	    pwm_response);
//    out_port #(.WIDTH(32)) u130 (32'h800000f1, clock, clock_100m, clock_valid,
//            reset, address_out, memory_drive, memory_write, bus, pwm_period);
//    out_port #(.WIDTH(32)) u131 (32'h800000f2, clock, clock_100m, clock_valid,
//            reset, address_out, memory_drive, memory_write, bus, pwm_high);
//
//    // SPI controller (untested since switching to turn protocol)
//    spi u132 (clock, clock_valid, reset, GPIO[4], GPIO[5], GPIO[6], GPIO[7],
//              spi_command, spi_response, spi_send_data, spi_receive_data);
//    turn_port u133 (32'h80000100, clock, clock, clock_valid, reset,
//	    address_out, memory_drive, memory_write, bus, spi_command,
//	    spi_response);
//    out_port #(.WIDTH(8)) u134 (32'h80000101, clock, clock, clock_valid, reset,
//            address_out, memory_drive, memory_write, bus, spi_send_data);
//    in_port #(.WIDTH(8)) u135 (32'h80000102, clock, spi_response, clock,
//            address_out, memory_drive, bus, spi_receive_data);

    control u136 (clock, clock_valid, reset, opcode_out, equal_out, lt_out,
